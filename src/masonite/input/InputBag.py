...
class InputBag:
    ...
    def parse(self, environ):
        ...
        if "wsgi.input" in environ:
            if "application/json" in environ.get("CONTENT_TYPE", ""):
                ...
                if request_body := environ["wsgi.input"].read(request_body_size):
                    self.post_data.update(
    ...
    def get(self, name, default=None, clean=True, quote=True):
        ...
        if isinstance(input, list) and len(input) == 1:
            return input if name.endswith("[]") else input[0]
    ...
    def all_as_values(self, internal_variables=False):
        ...
        return {
            ...
            if internal_variables or not name.startswith("__")
        }

    def only(self, *args):
        all = self.all()
        return {name: self.get(name) for name, input in all.items() if name in args}
